...
async function handleGameEnd(game: GameState, outcome: 'win' | 'lose' | 'push' | 'blackjack'): Promise<void> {
  if (outcome === 'blackjack') {
    addBalance(game.userId, Math.floor(game.bet * 2.5)); // Get back bet + 1.5x winnings
  } else if (outcome === 'win') {
    addBalance(game.userId, game.bet * 2); // Get back bet + winnings
  } else if (outcome === 'push') {
    addBalance(game.userId, game.bet); // Get back bet
  }
