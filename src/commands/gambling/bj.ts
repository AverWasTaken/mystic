// Alias for blackjack - imports and re-exports with different name
import { 
  Message, 
...
const command: Command = {
  name: 'bj',
  description: 'Play a game of blackjack (alias for blackjack)',
  ...
  async execute(message: Message, args: string[]): Promise<void> {
    if (args.length < 1) {
      await message.reply('❌ Usage: `m!bj <amount>` or `m!bj all`');
      return;
    }
